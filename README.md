...
**Running with a real LLM (e.g. DeepSeek):** The code is structured so it can run with a real LLM if you provide an API key. (1) Set one of: `DEEPSEEK_API_KEY` or `LLM_API_KEY` (environment variable). (2) Optionally set `LLM_BASE_URL` (default for DeepSeek is `https://api.deepseek.com`) and `LLM_MODEL` (default `deepseek-chat`). (3) Install the OpenAI-compatible client: `pip install openai`. Then run `python demo.py` as above. Demo 1 will use the LLM as the agent that decides to call the tool; demos 2–5 stay simulated so error handling and edge cases remain predictable.
